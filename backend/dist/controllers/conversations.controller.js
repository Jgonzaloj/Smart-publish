...
class ConversationsController {
    static async getConversations(req, res) {
        ...
        try {
            const [convRows] = await database_1.pool.query(`SELECT c.id, c.channel, c.status, c.last_message_at,
                        cust.name as customer_name, cust.phone as customer_phone
                 FROM conversations c
                 JOIN customers cust ON c.customer_id = cust.id
                 WHERE c.workspace_id = ?
                 ORDER BY c.last_message_at DESC`, [workspaceId]);
            if (convRows.length > 0) {
                const conversations = await Promise.all(convRows.map(async (conv) => {
                    const [msgRows] = await database_1.pool.query(`SELECT id, sender, message_text as text, 
                                    DATE_FORMAT(created_at, '%H:%i') as time
                             FROM messages
                             WHERE conversation_id = ?
                             ORDER BY created_at ASC`, [conv.id]);
    ...
    static async sendMessage(req, res) {
        ...
        const msgId = crypto_1.default.randomUUID();
        const now = new Date();
        const timeStr = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        try {
            await database_1.pool.query(`INSERT INTO messages (id, conversation_id, sender, message_text) VALUES (?, ?, ?, ?)`, [msgId, id, sender === 'CUSTOMER' ? 'CUSTOMER' : 'HUMAN_AGENT', text]);
            await database_1.pool.query(`UPDATE conversations SET last_message_at = NOW() WHERE id = ?`, [id]);
        }
        catch (err) {
    ...
    static async toggleAiMode(req, res) {
        const workspaceId = req.user?.workspace_id || req.workspaceId;
        const { id } = req.params;
        const { status } = req.body; // AI_HANDLED | HUMAN_NEEDED
        try {
            await database_1.pool.query(`UPDATE conversations SET status = ? WHERE id = ? AND workspace_id = ?`, [status, id, workspaceId]);
        }
        catch (err) {
