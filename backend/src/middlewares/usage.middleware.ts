...
export const checkAiUsageLimit = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const plan = (workspace.plan_id || 'free') as keyof typeof PLAN_LIMITS;
...
export const checkPostUsageLimit = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const workspaceId = (req as any).user.workspace_id;
        const workspace = await workspaceRepository.findById(workspaceId);
