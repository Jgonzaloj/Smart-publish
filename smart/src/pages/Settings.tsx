...
export const Settings = () => {
  ...
  const handleConnectTikTok = () => {
    setIsLoading(true);
    // As seen in tiktok.controller.ts, the backend redirects directly
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-10">
      ...
      {activeTab === 'integrations' && (
        ...
        <div className={`glass-panel overflow-hidden transition-all duration-300 relative group lg:col-span-2 lg:max-w-2xl
          ${isTikTokConnected ? 'border-zinc-200 dark:border-zinc-800 ring-1 ring-zinc-500/20 shadow-lg shadow-zinc-500/10' : 'hover:border-slate-300 dark:hover:border-slate-600'}
        `}>
          <div className="p-6 relative z-10 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
            <div className="w-16 h-16 rounded-full bg-black dark:bg-white text-white dark:text-black flex items-center justify-center shadow-lg shrink-0 border-2 border-white dark:border-slate-800">
              <Share2 size={32} />
            </div>
            <div className="flex-1 text-left">
              ...
              <p className="text-slate-600 dark:text-slate-400 text-sm mb-4">
                Programa tus videos de formato corto directamente a tu feed de TikTok utilizando la API oficial de creadores.
              </p>
              
              {isTikTokConnected && (
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700/50 space-y-3 w-full max-w-sm">
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Cuentas vinculadas</div>
                  {tiktokAccounts.map(acc => (
                    <div key={acc.id} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-black/10 dark:bg-white/10 text-black dark:text-white flex items-center justify-center">
                          <Share2 size={14} />
                        </div>
                        <span className="font-semibold text-sm">{acc.account_name}</span>
                      </div>
                  ...
                  <div className="pt-3 mt-3 border-t border-slate-200 dark:border-slate-700">
                    <button className="text-red-500 hover:text-red-600 text-sm font-semibold flex items-center gap-1 transition-colors">
                      <Unplug size={16} /> Desconectar TikTok
                    </button>
                  </div>
                </div>
              )}
            ...
            {!isTikTokConnected && (
              <div className="shrink-0 w-full sm:w-auto mt-4 sm:mt-0">
                <button 
                  ...
                  disabled={isLoading}
                  className="w-full sm:w-auto btn-primary bg-black hover:bg-zinc-800 dark:bg-white dark:hover:bg-zinc-200 dark:text-black text-white justify-center px-8"
                >
                  {isLoading ? <Loader2 className="animate-spin" size={20} /> : 'Conectar'}
                </button>
              </div>
            )}
          </div>
