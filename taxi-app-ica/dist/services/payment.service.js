...
export class PaymentService {
     ...
     */
    processRidePayment(rideId, amount, method, operationCode) {
    ...
    `);
        ...
        const ride = this.db.prepare('SELECT driver_id FROM rides WHERE id = ?').get(rideId);
        if (ride?.driver_id) {
            if (method === 'cash') {
        ...
        `).run(commissionAmount, ride.driver_id);
                ...
                const dRow = this.db.prepare('SELECT wallet_balance FROM drivers WHERE user_id = ?').get(ride.driver_id);
     ...
     */
    generateYapePaymentInfo(rideId, amount, driverPhone = '956987111', driverName = 'Mario Huamán García') {
     ...
     */
    generatePlinPaymentInfo(rideId, amount, driverPhone = '956987111', driverName = 'Mario Huamán García') {
