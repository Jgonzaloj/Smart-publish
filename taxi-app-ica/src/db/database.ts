...
import { config, ICA_LANDMARKS } from '../config/env.js';
...
function seedInitialData(db: Database.Database): void {
  ...
  const passengerCount = db.prepare("SELECT count(*) as count FROM users WHERE role = 'passenger'").get() as { count: number };
