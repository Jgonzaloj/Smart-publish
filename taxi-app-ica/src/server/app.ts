...
import { authMiddleware, generateToken, AuthenticatedRequest } from '../middleware/auth.middleware.js';
...
app.post('/api/rides/request', authMiddleware(['passenger', 'admin']), (req: AuthenticatedRequest, res) => {
...
app.post('/api/rides/:id/bid', authMiddleware(['driver', 'admin']), (req: AuthenticatedRequest, res) => {
...
app.post('/api/rides/:id/accept-bid', authMiddleware(['passenger', 'admin']), (req: AuthenticatedRequest, res) => {
...
app.post('/api/rides/:id/cancel', authMiddleware(['passenger', 'driver', 'admin']), (req: AuthenticatedRequest, res) => {
  const { reason } = req.body;
  const rideId = req.params.id as string;
  const ride = dispatchService.updateRideStatus(rideId, 'CANCELLED');
  if (!ride) return res.status(400).json({ success: false, message: 'No se pudo cancelar el viaje' });
...
app.post('/api/rides/:id/rate', authMiddleware(['passenger', 'driver', 'admin']), (req: AuthenticatedRequest, res) => {
...
app.post('/api/drivers/register', (req, res) => {
  const {
    full_name, phone, email, dni, plate_number, brand, model, color, year,
    soat_number, soat_expiry, license_number, license_expiry,
    technical_review_number, technical_review_expiry, property_card, auto_approve,
  } = req.body;
...
app.post('/api/drivers/:id/documents', authMiddleware(['driver', 'admin']), (req: AuthenticatedRequest, res) => {
  const {
    property_card,
    property_card_photo,
    soat_number,
...
app.post('/api/auth/verify-otp', (req, res) => {
  ...
  `).get(cleanPhone, code.toString().trim()) as any;
